...
function GetQuestionList(item) {
    ...
    if (item.value !== null) {
        ...
        var SubId1 = item;
        var IdintSubId1 = parseInt(SubId1);
        $("#subTestId").val(IdintSubId1);
    }
    var count = 1;
    $.ajax
    ({
        ...
        success: function (result) {
            ...
            $.each(result, function (index, value) {
                ...
                if (value.OpOne.length > 10) {
                    subop1 = value.OpOne.substring(0, 10) + '.....';
                } else {
                    subop1 = value.OpOne;
                }
                if (value.OpTwo.length > 10) {
                    subop2 = value.OpTwo.substring(0, 10) + '.....';
                } else {
                    subop2 = value.OpTwo;
                }
                if (value.OpThree.length > 10) {
                    subop3 = value.OpThree.substring(0, 10) + '.....';
                }
                else {
                    subop3 = value.OpThree
                }
                if (value.OpFour.length > 10) {
                    subop4 = value.OpFour.substring(0, 10) + '.....';
                }
                else {
                    subop4 = value.OpFour;
                }
                if (value.Correct.length > 10) {
                    subcor = value.Correct.substring(0, 10) + '.....';
                }
                else {
                    subcor = value.Correct;
                }
